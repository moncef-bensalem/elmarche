        ...
        methods: {
            ...
            purchase: function() {
                ...
                Object.values(this.error).forEach(values => {
                    if (values) {
                        is_error = 1;
                        console.log('Erreur de validation trouvée:', values);
                    }
                });

                if (!is_error) {
                    console.log('Validation réussie, création du token Stripe');
                    this.toggleLoader();
                    ...
                    const additional_data = {
                        ...
                        address_zip: billing_details.postal_code,
                        address_country: billing_details.country,
                        currency: "usd"
                    };
                    
                    console.log('Envoi des données à Stripe:', additional_data);
                    ...
                    stripe.createToken(card, additional_data).then(function(result) {
                        ...
                        console.log('Token Stripe créé avec succès');
                        const order = {
            ...
            addToServer(order) {
                ...
                this.newOrder(order).then(res => {
                    ...
                    if (res.status === 201) {
                        ...
                        console.error('Erreur lors de la création de la commande:', res);
                        this.toggleLoader();
                    }
                }).catch(error => {
